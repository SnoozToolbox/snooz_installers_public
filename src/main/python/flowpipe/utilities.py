...
def deserialize_node(data):
    ...
    module = data["module"]
    classname = data["module_name"]
        
    node = import_class(module, classname)(graph=None)
...
def deserialize_graph(data):
    ...
    for node in data['nodes']:
        if node[ActivationState.KEY] != ActivationState.DEACTIVATED:
    ...
    all_nodes = [n for n in data['nodes'] if n[ActivationState.KEY] != ActivationState.DEACTIVATED]
    ...
    for node in all_nodes:  # data['nodes']:
        ...
        for name, input_ in node['module_inputs'].items():
