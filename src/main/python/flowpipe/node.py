...
class INode(object):
    ...
    def set_pub_sub_manager(self, pub_sub_manager):
        """ Set the PubSubManager"""
    ...
    def evaluate(self):
        ...
        state_change_topic = f"{self.identifier}.state_change"

        self.EVENTS['evaluation-started'].emit(self)
        self._pub_sub_manager.publish('graph', state_change_topic, 'is_running')
        inputs = {}
        for name, plug in self.inputs.items():
            inputs[name] = plug.value

        # Compute and redirect the output to the output plugs
        self._log_manager.log(self.identifier, f"-------------------")
        self._log_manager.log(self.identifier, f"{self.name}")
        self._log_manager.log(self.identifier, f"{self.identifier}")
        
        start_time = time.time()
        try:
            outputs = self.compute(**inputs) or dict()
        except Exception as e:
            self._log_manager.log(self.identifier, f"{traceback.format_exc()}")
            self._pub_sub_manager.publish('graph', state_change_topic, 'is_failed')
            raise e
        ...
        fstart_time = datetime.fromtimestamp(start_time).strftime("%A, %B %d, %Y %I:%M:%S")
        feval_time = dt.timedelta(seconds=eval_time)
        self._log_manager.log(self.identifier, f"Start time:\t{fstart_time}")
        self._log_manager.log(self.identifier, f"Process time:\t{feval_time}")
        self._pub_sub_manager.publish('graph', state_change_topic, 'is_done')
