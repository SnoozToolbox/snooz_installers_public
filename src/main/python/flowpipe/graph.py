...
#Uncomment to see the logs in the terminal
#logging.basicConfig()
...
if DEBUG:
    # Get current time as string in format "YYYY-MM-DD_HH:MM:SS.SSS"
    current_time = time.strftime("%Y-%m-%d_%H-%M-%S")
    logging.basicConfig(filename=f"execution_{current_time}.log", encoding='utf-8', level=logging.DEBUG)
...
class Graph(object):
    ...
    def evaluation_matrix(self):
        ...
        # Inline recursive function to parse the tree of nodes and assign
        # the order at which they are safe to be executed based on their relation
        # to their parent's order.
        def recursive_set_order(node, order):
            node.order = order
            # For each output
            for key, outputPlug in node.outputs.items():
                # For each connection
                for connection in outputPlug.connections:
                    # if output.order is not defined or lower than it now needs to be,
                    # call the recursive function to change it and change it's children
                    if not hasattr(connection.node, "order") or \
                        connection.node.order < order + 1:
                        recursive_set_order(connection.node, order + 1)

        # For each root nodes
        for node in root_nodes:
            order = 0
            recursive_set_order(node, order)

        levels = {}
        for node in self.all_nodes:
            levels[node] = node.order
    ...
    def evaluate(self, mode="linear", skip_clean=False,
        ...
        # Send a message to init the visual states of all nodes
        for node in nodes_to_evaluate:
            node._pub_sub_manager.publish('graph', f"{node.identifier}.state_change", 'is_waiting')
    ...
    def _evaluate_linear(self, nodes_to_evaluate):
        ...
        for node in nodes_to_evaluate:
            log.debug("Evaluating node {0}.".format(
            node.name))
            node.evaluate()
    ...
    def _evaluate_threaded(self, nodes_to_evaluate, max_workers=None):
        ...
        def node_runner(node):
            ...
            node.name))
            node.evaluate()
            log.debug("Evaluation Completed{0}.".format(
            node.name))
            return node
            ...
            while nodes_to_evaluate or running_futures:
                ...
                for node in nodes_to_evaluate:
                    if not any(n.is_dirty for n in node.upstream_nodes):
                        ...
                        running_futures[node.identifier] = fut
                    else:
                ...
                for s in status.done:
                    ...
                    del running_futures[s.result().identifier]
